...
DAP（先輩担当者モード）に収録されている 6 本のデモシナリオに沿って画面収録します。  
DAP チャットで「**デモ**」「**DEMO2**」などと入力すると、対応するフローが自動起動します。

> **起動 URL**: https://dap.tsp-aitrademanagement.com

---

### DEMO1：AI 輸出審査クイックデモ（5 分）

**品目**: レーザー変位センサー LS-500 / 仕向地: 台湾 TSMC  
**ポイント**: スクリーニング → AI 取引審査 → 判定結果を最短フローで体験

```
Step 1: システム概要 & デモ開始
  → DAP チャットで「デモ」と入力してフロー開始

Step 2: スクリーニング（制裁照合）
  → http://localhost:8005/ui でTSMC を入力・照合実行
  → 結果: クリア（Green）

Step 3: AI 取引審査フォーム入力
  → http://localhost:8011/ui/transactions/new
  → LS-500 / 台湾 / TSMC / 半導体製造向け を入力

Step 4: AI 判定結果・証跡確認
  → 「AI 解析を実行」→ 2リスト判定・キャッチオール評価
  → 判定根拠・証跡の PDF 出力を確認
```

---

### DEMO2：キャッチオール規制リスク検知（3 分）

**品目**: 汎用品（無規制品目）/ 仕向地: 中国  
**ポイント**: リスト規制非該当でも、キャッチオール評価で WMD 転用懸念を検知
...
Step 2: リスク案件の入力
  → http://localhost:8011/ui/transactions/new
  → 中国向け・軍民両用懸念品目のサンプルデータが自動入力される

Step 3: リスク検知結果の確認
  → キャッチオール評価で「要注意」フラグが付与されることを確認
  → ERP への NEEDS_REVIEW 通知を確認
```

---

### DEMO3：品目登録 → BOM → 正式審査 → 輸出許可 完全ワンストップ（15 分）

**品目**: 精密光学計測システム OMEGA-300 / 仕向地: 台湾 TSMC  
**ポイント**: 品目起票からライセンス申請まで一切手戻りなしの完全フロー

```
Step 1: シナリオ概要・全体フロー
  → DAP チャットで「DEMO3」と入力

Step 2: Finished Goods 品目登録（OMEGA-300）
  → http://localhost:8002/products/new → サンプルデータ自動入力

Step 3: BOM 構成品 4 点を直接追加・SC 同期
  → BOM 追加画面 → サプライチェーンノード自動生成

Step 4: SC で BOM ツリー確認・サプライヤー証明収集
  → BOM グラフ可視化 → 仕入先ポータルへの証明収集依頼

Step 5: AI 取引審査フォーム入力（7 項目）
  → http://localhost:8011/ui/transactions/new → OMEGA-300 データ自動入力

Step 6: AI 判定実行・結果確認
  → NeuroSymbolic エージェント判定 → ECCN 3B001 確認

Step 7: キャッチオール判定・正式審査提出・輸出許可申請
  → 正式審査提出 → 輸出許可申請書（BIS-748P 形式）ドラフト生成
```

---

### DEMO4：品目マスター完全セットアップ — HS・AI 該非・国別関税（8 分）

**品目**: 精密温度センサー PT-300  
**ポイント**: 品目登録から 4 カ国の国別規制プロファイルまで品目マスタを完全構築
...
Step 2: 品目登録
  → http://localhost:8002/products/new → PT-300 データ入力

Step 3: HS コード AI 判定
  → http://localhost:8006/ → 品目説明から HS コード候補を自動サジェスト（5,476 vec）

Step 4: 外部 AI 該非判定（Webhook 連携）
  → 「外部AI判定依頼」→ NeuroSymbolic エージェントが ECCN サジェスト

Step 5: 判定項番転記・判定記録保存
  → 担当者が確認・確定 → 証跡として保存

Step 6: 国別プロファイル登録（US / CN / ID / EU）
  → 各国 HS コード・MFN 関税率・EPA 優遇税率を一括登録
```

---

### DEMO5：R&D リスク管理 — みなし輸出・外国人研究者審査（8 分）

**人物**: 陳 雷（中国籍研究者）/ 研究: GaN-HEMT 次世代プロセス  
**ポイント**: 外為法第 25 条みなし輸出審査の全フローを実演

```
Step 1: みなし輸出とは — シナリオ概要
  → DAP チャットで「DEMO5」と入力

Step 2: 人物登録 — 外国籍研究者の登録
  → http://localhost:8003/ui → 陳 雷（中国籍・Tsinghua University）を登録

Step 3: スクリーニング結果確認
  → 制裁リスト照合 → 機関リスクスコア確認

Step 4: R&D 案件起案
  → 「新規プロジェクト」→ GaN-HEMT 研究プロジェクト作成

Step 5: プロファイル入力 & AI リスク評価
  → 技術機密性・仕向国リスク・関係者属性を入力
  → AIリスクスコア算出 → HIGH 判定 → 取引審査案件自動生成
```

---

### DEMO6：役務取引管理 — みなし輸出サービス取引審査（5 分）

**取引内容**: 技術提供・ソフトウェアライセンス供与（外為法第 25 条）  
**ポイント**: 物の輸出ではなく「技術・役務」の提供に対する審査フロー

```
Step 1: 役務取引（みなし輸出）とは — シナリオ概要
  → DAP チャットで「DEMO6」と入力

Step 2: 役務案件一覧 — 管理画面の確認
  → http://localhost:8003/ui → 役務取引タブ

Step 3: 新規役務案件登録 — 相手方情報と技術概要を入力
  → 技術提供先（国・機関）・提供技術の概要を入力

Step 4: スクリーニング結果確認 — OFAC/BIS 制裁リストチェック
  → 自動スクリーニング実行 → 結果確認

Step 5: 審査提出・許可判定記録
  → 審査提出 → AI 判定結果の記録・証跡保存
```
