...
# 2026-08-16 更新（IF-22・AI判定自動実行 完了・全49テストグリーン）

> **2026-08-16 追加: IF-22（返品・再輸出の審査）+ AI判定自動実行 実装完了** — ①`POST /api/transactions/{tx_id}/return-review` 追加（`disposition` 3分岐: `scrap`=廃棄記録 / `return_to_origin`=輸入規制確認案内 / `reship_to_other`=新TX RET-YYYYMM-NNNNNN 自動起票）②`POST /api/licenses/allocations/{allocation_no}/return` 追加（ライセンスクォータ rollback: `consumed_value_usd`/`consumed_unit` 差し戻し）③`_run_matrix_match_bg(tx_id)` 実装（IF-01 新規仮審査 / IF-02 非smooth本審査 の起票後に AI パイプライン5ステップを daemon スレッドで自動実行）④統合テスト `test_if22_ai_auto.py` IF22-00〜06 + AUTO-01〜03（10/10 GREEN）。これにより **全49テスト（35+14）グリーン**。

> **2026-08-16 追加: 3システム連携 Phase 5（継続モニタリング）完了** — ①`monitoring_subscription` テーブル（Alembic `s8t9u0v1w2x3`、platform-core PostgreSQL、部分インデックス `WHERE is_active=TRUE` でユニーク制約）②`MonitoringSubscription` ORM モデル・`/api/monitoring/subscriptions` CRUD API（POST/GET/DELETE）③`POST /api/parties/resolve` に `enable_watch: bool`・`monitor_until: date` パラメータ追加（IF-01 時に自動購読作成）④`_monitoring_worker` バックグラウンドタスク（24時間周期: 期限切れ購読自動無効化 → party 再スクリーニング → sanction_status 変化検知 → `WebhookDispatcher.enqueue` 経由で IF-16/IF-23 通知）⑤統合テスト MT-01〜06（6/6 GREEN）。これにより **3システム連携 Phase 0〜5 全フェーズ完了**（39/39 テスト全グリーン）。

> **2026-08-15 追加: 3システム連携（ERP/AI_TM/CRM）設計レビュー完了** — `AI_TM_連携引き継ぎ書.md`（2,195行）を精読・現状コードベースとのギャップ全分析完了。成果物: `docs/DEVELOPMENT_PLAN_3SYSTEM_INTEGRATION.md`（144pd / Phase 0〜5 詳細タスクリスト）。主要発見: ①HMAC認証がコードベース全体でゼロ（Phase 0 A0-2）②Webhook Dispatcher 完全新規（Phase 0 A0-4〜7）③Party Registry / 名寄せロジック未実装（Phase 2、最大工数35pd）④export_license モジュールは Phase 4 で SQLite → PostgreSQL 移行が必須（行レベルロック要件）⑤ERP回帰テストを Phase 1 着手前に先行作成すること。アーキテクチャ5決定事項を実装前に確定することが推奨。

> **2026-08-02 追加: AI 該非判定 UX & スコアリング刷新** — ①`_context_score()`関数追加（FAISS 0.83〜0.87 均一スコア → 実務的 0.2〜1.0 スコアに変換: 再正規化・ソース種別重み・汎用ラベルペナルティ・ECCN一致 2.8x ボーナス）②`POST /api/compliance/hantei` 新エンドポイント（3ティア分類: 要精査 adj>=0.65・要確認 0.35<=adj<0.65・参考<0.35、item_no 重複排除）③AI 法令チェックパネル完全リデザイン（「▶ リスク評価」+「🎯 該非候補を取得」の2ボタン構成、3ティア折りたたみ表示）④`transferFromAI()`をティア別確認方式に変更（prompt選択: 要精査のみ/要精査+要確認）⑤`window.renderRows`グローバル公開（スコープ外 IIFE からの呼び出し対応）⑥`_build_regulation()`/`_build_fefta()` を文脈考慮スコア使用に更新（CVD装置 regulation score 0.873→0.444 と意味のある分布に）

> **2026-07-27 追加: Phase 23-29 + AI コンプライアンスリスク評価エンジン完了** — ①Layer A +1,649vec（CCL ECCNテキスト詳細チャンク: items_controlled/license_requirements/technical_notes、計3,982vec）②Layer D +732件（OpenAlex学術論文: 3A090/量子/GaN/合成生物 2022-2025、計6,497vec）③Layer E +20件（戦略政策文書 POL-093〜112: BIS/OFAC執行事例・METI・EU AI Act・量子・CHIPS FEOC・みなし輸出・CFIUS、計173vec）④Layer B ECCN→FEFTA逆引き補完（fterm_eccn_mapping.json 68 ECCN対応、FEFTA未設定特許に自動付与）⑤Layer C FEFTA+70件（word-boundary keyword matching導入、2,876→2,946 mapped）⑥Ontology v2.0（nx_graph 2,524→2,544ノード・6,341→6,374辺、20 P3-2ノード・33 regime_to_eccn辺追加）⑦Layer F 新設（制裁執行事例21vec: OFAC SDN/BIS Denial/ITAR同意令/METI処分、`GET /api/faiss/search/layer-f`・DAP `_rag_sanctions_cases()` 統合）⑧`POST /api/compliance/assess` 新設（5軸: CCL/ECCN規制・外為法・制裁執行事例・仕向国リスク・先端技術論文、総合リスクスコア+推奨アクション）⑨AI リスク評価パネルを取引詳細(8011)/品目編集(8002)/R&Dケース(8003)/スクリーニング(8005) の各UI に追加（高リスク国は自動実行）⑩FAISS startup preload追加（起動直後の初回レイテンシ解消）⑪`/health` に FAISS 6レイヤー状態表示追加⑫`scripts/check_system.py` 新設（17項目診断・1秒完了）
