...
app = FastAPI(title="DAP Coach Server (Admin + Runtime)")

# Local dev: allow all origins (Chrome extension + localhost apps)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def runtime_config(app_key: str, env: str = "local", db: Session = Depends(get_db)):
    ...
    if not rel or not rel.runtime_config:
        ...
        etag = stable_etag(cfg)
        return JSONResponse({"env": env, "version": "live", "config": cfg, "etag": etag})

    etag = stable_etag(rel.runtime_config)
    return JSONResponse({"env": env, "version": rel.version, "config": rel.runtime_config, "etag": etag})
...
def list_pages(app_id: int, db: Session = Depends(get_db)):
    ...
    return [
        {"id": p.id, "app_id": p.app_id, "page_key": p.page_key, "name": p.name,
         "url_regex": p.url_regex, "must_have": p.must_have or {}}
        for p in pages
    ]
...
def upsert_target(payload: TargetUpsert, db: Session = Depends(get_db)):
    existing = (
        db.query(DapTarget)
        .filter(DapTarget.page_id == payload.page_id, DapTarget.target_key == payload.target_key)
        .one_or_none()
    )
    if existing:
        existing.description = payload.description
        existing.anchors = payload.anchors
        db.add(existing)
        db.commit()
...
def create_intervention(payload: InterventionCreate, db: Session = Depends(get_db)):
    iv = DapIntervention(
        ...
        rule_key=payload.rule_key,
        block_action=payload.block_action,
        coach=payload.coach,
...
def update_intervention(iv_id: int, payload: InterventionUpdate, db: Session = Depends(get_db)):
    ...
    iv.name = payload.name
    iv.trigger = payload.trigger
    iv.rule_key = payload.rule_key
    iv.block_action = payload.block_action
    iv.coach = payload.coach
    iv.actions = payload.actions
    db.add(iv)
    db.commit()
    db.refresh(iv)
...
def update_page(page_id: int, payload: PageUpdate, db: Session = Depends(get_db)):
    ...
    p.name = payload.name
    p.url_regex = payload.url_regex
    p.must_have = payload.must_have
    db.add(p)
    db.commit()
    return {"ok": True}
...
def get_latest_release(app_id: int, env: str = "local", db: Session = Depends(get_db)):
    rel = (
        db.query(DapRelease)
        .filter(DapRelease.app_id == app_id, DapRelease.env == env, DapRelease.status == "published")
        .order_by(DapRelease.created_at.desc())
        .first()
    )
    if not rel:
        return {"version": None}
    return {"version": rel.version, "id": rel.id}
...
def publish(payload: PublishAutoIn, db: Session = Depends(get_db)):
    ...
    # Auto-increment minor version from latest published release
    latest = (
        db.query(DapRelease)
        .filter(DapRelease.app_id == payload.app_id, DapRelease.env == payload.env, DapRelease.status == "published")
        .order_by(DapRelease.created_at.desc())
        .first()
    )
...
def recorder_target(payload: dict, db: Session = Depends(get_db)):
    ...
    payload:
    {
      "app_key": "rd_risk_local",
      "page_key": "rd_case_form",
      "target_key": "t_enduse",
      "description": "用途要件",
      "anchors": [{"strategy":"attr","value":"data-dap-field=end_use_description"}, ...]
    }
    """
...
def analytics_summary(app_key: str, db: Session = Depends(get_db)):
    ...
    return [
        {"ts": r.ts, "page_id": r.page_id, "event_type": r.event_type, "event_name": r.event_name, "context": r.context}
        for r in rows
    ]
