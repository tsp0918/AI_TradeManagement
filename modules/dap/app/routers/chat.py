...
【ルール】
- reply: マークダウン禁止（**や# など使わない）。口語体日本語。
  ・業務操作・ナビゲーション系の質問 → 100字以内で端的に答える。
  ・説明系の質問（「なぜ」「背景」「理由」「哲学」「意味」「歴史」「目的」「そもそも」「重要性」等）→ 400〜800字で実際の内容をこのフィールドに直接書く。知識ベース情報を引用しながら説明する。
- 【絶対禁止パターン】「〜の観点から説明します」「3つの層で説明します」「第1層：」などと構造を予告するだけで内容を choices やボタンに先送りにすること。説明を求められたら reply に直接内容を書く。choices は「次に何を深掘りするか」の提示であって、今の質問の回答を代替するものではない。
- 説明系質問への choices: 内容の続きを聞くもの（「実務への影響を教えて」等）または実際の業務アクションへの接続（「みなし輸出審査を始める」等）にする。「哲学的背景を聞く」「地政学的背景を聞く」のような現在の質問と同じトピックを繰り返す choices は禁止。
- actions: 「画面上のボタン・リンク」リストの要素が該当する場合は必ず含める。target は完全一致。
- choices: 必ず2〜3件含める（省略・空配列禁止）。
...
async def chat(req: ChatRequest, db: Session = Depends(get_db)) -> ChatResponse:
    ...
    _RESPOND_TOOL = {
        ...
        "description": (
            ...
            "reply には実際の回答内容を必ず書くこと。"
            "業務操作・ナビゲーション系は100字以内で簡潔に。"
            "「なぜ」「背景」「哲学」「理由」「説明して」等の説明系質問には400〜800字で実質的な内容を直接書く。"
            "構造の予告だけして内容を choices に先送りするのは絶対禁止。"
            "interactive_elements リストに一致する要素があれば必ず actions に highlight を含める。"
            "choices は常に2〜3件含めること（必須）。"
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "reply": {
                    "type": "string",
                    "description": (
                        "ユーザーへの返答テキスト。マークダウン(**や#など)禁止。口語体日本語。"
                        "【業務操作・ナビゲーション系】100字以内で端的に。"
                        "【説明・背景・理由系（「なぜ」「背景」「哲学」「意味」「理由」「説明して」等）】"
                        "400〜800字で実際の内容を直接この reply フィールドに書くこと。"
                        "「〜について説明します」「〜観点から順に説明します」と予告だけして"
                        "内容を choices に回すことは絶対禁止。知識ベースの情報を引用しながら直接答える。"
                    ),
                },
                "actions": {
                    "type": "array",
