...
def _build_draft_content_bis748p(data: dict) -> dict:
    """BIS-748P フォームフィールドを構築する。"""
    return {
        # Block 1: U.S. Principal Party in Interest
        ...
        # Block 4: Type of Application
        "block4_application_type": "Individual Validated License (IVL)",
        # Block 7: Country of Ultimate Destination
...
async def create_license(body: LicenseCreate, db: AsyncSession = Depends(get_db)):
    ...
    draft = _build_draft_content_bis748p({
    ...
    }) if body.license_type == "EAR" else _build_draft_content_fefta({
        "destination_country": body.destination_country,
        "consignee_name": body.consignee_name,
        "consignee_address": body.consignee_address,
        "end_user_name": body.end_user_name,
...
async def get_license(app_id: str, db: AsyncSession = Depends(get_db)):
    a = await _get_or_404(app_id, db)
    return _serialize(a)


@router.put("/api/export-licenses/{app_id}")
async def update_license(app_id: str, body: LicenseCreate, db: AsyncSession = Depends(get_db)):
    a = await _get_or_404(app_id, db)
...
async def delete_license(app_id: str, db: AsyncSession = Depends(get_db)):
    a = await _get_or_404(app_id, db)
    await db.delete(a)
    await db.commit()
...
async def submit_license(app_id: str, db: AsyncSession = Depends(get_db)):
    a = await _get_or_404(app_id, db)
    a.status = "submitted"
...
async def approve_license(app_id: str, body: LicenseApprove, db: AsyncSession = Depends(get_db)):
    a = await _get_or_404(app_id, db)
    a.status = "approved"
...
async def deny_license(app_id: str, db: AsyncSession = Depends(get_db)):
    a = await _get_or_404(app_id, db)
    a.status = "denied"
    await db.commit()
    await db.refresh(a)
...
async def draft_from_transaction(body: DraftFromTransaction, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        pass  # 取引が取れない場合は空ドラフトで続行

    item_desc = tx_data.get("item_name") or tx_data.get("item_description") or ""
    eccn = tx_data.get("eccn") or ""
    dest = tx_data.get("destination_country") or ""
    consignee = tx_data.get("consignee_name") or tx_data.get("end_user") or ""
    end_use = tx_data.get("end_use") or tx_data.get("transaction_purpose") or ""
...
async def preview_license(app_id: str, db: AsyncSession = Depends(get_db)):
    """BIS-748P / 外為法 様式第1 の HTML フォームプレビューを返す。"""
    a = await _get_or_404(app_id, db)
    d = a.draft_content or {}

    if a.license_type == "EAR":
        html = _render_bis748p(a, d)
    else:
        html = _render_fefta(a, d)
