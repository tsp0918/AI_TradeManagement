...
- POST /api/item-versions/sync-preview               ERP 同期プレビュー（品目同期ボタン用）
...
_FAISS_URL        = "http://localhost:8000"   # self (platform-core)
_ERP_ADAPTER_URL  = "http://localhost:5001"
_REVIEW_VALID_DAYS = 365                      # 審査有効期間
...
class CheckAndLinkResponse(BaseModel):
    ...
    approved: bool
    linked_existing: bool          # True=既存審査に紐づけ, False=新規審査
    eccn: str | None = None
...
async def _ensure_company(
    ...
    """plat_company を upsert して返す（テナント: default）。"""
...
async def get_stats(db: AsyncSession = Depends(get_db)) -> dict:
    ...
    approved = (await db.execute(
        select(func.count()).where(TransactionReview.approved == True)
    )).scalar() or 0
    needs_review = (await db.execute(
        select(func.count()).where(TransactionReview.judgment == "NEEDS_REVIEW")
    )).scalar() or 0
    pending = (await db.execute(
        select(func.count()).where(TransactionReview.review_completed == False)
    )).scalar() or 0
...
async def check_and_link(
    ...
    """
    # ── 既存審査を検索（同一品目 + 同一仕向地で有効期限内かつ完了済み）────────
    existing = (await db.execute(
        select(TransactionReview).where(
            ...
            TransactionReview.review_completed == True,
            TransactionReview.approved == True,
    ...
    item    = await _ensure_item(
        ...
        ai_result.get("eccn") or body.eccn,
        body.hs_code,
        ai_result.get("judgment"),
    )
...
async def get_review(review_id: str, db: AsyncSession = Depends(get_db)) -> dict:
...
async def shipment_rescreen(
    ...
    """
    ③ 出荷伝票受付: 紐づいた審査レコードを再スクリーニングし、
       変化がなければ approved=True を返す。
       変化がある場合は NEEDS_REVIEW に戻し手動確認を要求する。
    """
    r = await db.get(TransactionReview, uuid.UUID(review_id))
    if not r:
        raise HTTPException(404, "審査レコードが見つかりません")
    ...
    # 再スクリーニング実行
    rescreen = await _screen_counterparty(
        r.company_name or "", r.destination_country or "", None
    )
...
async def sync_preview(db: AsyncSession = Depends(get_db)) -> dict:
    """
    ERP 同期ボタン押下前プレビュー。
    ECCN または export_control_status が設定されている品目の件数を返す。
    """
    total   = (await db.execute(select(func.count(Item.id)))).scalar() or 0
    with_eccn = (await db.execute(
        select(func.count()).where(Item.eccn != None)
    )).scalar() or 0
    items = (await db.execute(
        select(Item).where(Item.eccn != None).limit(20)
    )).scalars().all()
...
async def sync_to_erp(
    ...
    """
    ECCN が設定された全品目を ERP にプッシュする（ERP 連携ボタン）。
    ERP アダプター経由で /internal/push-judgment を呼び出す。
    """
    items = (await db.execute(
        select(Item).where(Item.eccn != None)
    )).scalars().all()
...
async def _push_items_to_erp(items: list[dict]) -> None:
    """ERP アダプターの /internal/push-judgment を品目ごとに呼び出す。"""
