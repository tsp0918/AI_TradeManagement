"""サプライヤー原産性証明 API ルーター。

エンドポイント:
- GET  /api/supplier-attestations                    一覧（node_id/status フィルタ）
- POST /api/supplier-attestations                    証明申告登録
- GET  /api/supplier-attestations/{id}               詳細
- PUT  /api/supplier-attestations/{id}               更新
- DELETE /api/supplier-attestations/{id}             削除
- POST /api/supplier-attestations/{id}/ai-validate   AI 検証実行（FAISS Layer A）
- POST /api/supplier-attestations/{id}/accept        承認
- POST /api/supplier-attestations/{id}/reject        却下
- GET  /api/supply-chain/nodes/{node_id}/attestations  ノード別証明一覧
"""
...
_PLATFORM_CORE_URL = "http://localhost:8000"
...
def _verdict(claimed: str | None, suggested: str | None, confidence: float) -> str:
    """申告 ECCN と AI 提示 ECCN を比較して verdict を返す。"""
    if not suggested:
        return "unverifiable"
    if not claimed or claimed.upper() in ("EAR99", "NLR", ""):
        # 申告が EAR99 なのに AI が管理品と示唆
...
# ── ノード別一覧（supply_chain router から呼び出す補完エンドポイント） ──────

@router.get("/api/supply-chain/nodes/{node_id}/attestations")
async def list_node_attestations(node_id: str, db: AsyncSession = Depends(get_db)):
...
async def list_attestations(
    ...
    filtered = []
    for a in items:
        if node_id and str(a.node_id) != node_id:
            continue
        if status and a.status != status:
            continue
        if ai_verdict and a.ai_verdict != ai_verdict:
            continue
        filtered.append(a)
    total = len(filtered)
    return {"total": total, "items": [_serialize(a) for a in filtered[offset: offset + limit]]}


# ── 登録 ─────────────────────────────────────────────────────────

@router.post("/api/supplier-attestations", status_code=201)
async def create_attestation(body: AttestationCreate, db: AsyncSession = Depends(get_db)):
...
async def get_attestation(attest_id: str, db: AsyncSession = Depends(get_db)):
    r = await db.execute(
...
async def update_attestation(
    attest_id: str, body: AttestationUpdate, db: AsyncSession = Depends(get_db)
):
    r = await db.execute(
        select(SupplierAttestation).where(SupplierAttestation.id == uuid.UUID(attest_id))
    )
    ...
    if body.attestation_date is not None:
        try:
            a.attestation_date = date.fromisoformat(body.attestation_date)
        except ValueError:
            pass
    if body.expiry_date is not None:
        try:
            a.expiry_date = date.fromisoformat(body.expiry_date)
        except ValueError:
            pass
    await db.commit()
    await db.refresh(a)
...
async def delete_attestation(attest_id: str, db: AsyncSession = Depends(get_db)):
    r = await db.execute(
        select(SupplierAttestation).where(SupplierAttestation.id == uuid.UUID(attest_id))
    )
    a = r.scalar_one_or_none()
...
async def ai_validate(attest_id: str, db: AsyncSession = Depends(get_db)):
    """FAISS Layer A で申告 ECCN を AI 検証する。

    - ノードの品名・説明を使って Layer A を検索
    - 上位ヒットの ECCN と申告 ECCN を比較
    ...
    # FAISS Layer A 検索（platform-core 内部呼び出し）
    hits: list[dict[str, Any]] = []
    ai_review_detail: dict[str, Any] = {"query": query}
    try:
        from platform_core.services.faiss_e5_service import search_layer_a, is_ready, preload
        if not is_ready():
    ...
    # 上位ヒットから suggested_eccn を抽出
    suggested_eccn: str | None = None
    confidence: float = 0.0
    if hits:
        top = hits[0]
        confidence = top["score"]
        # item_no が ECCN 形式（例: "3A001"）なら採用
...
async def accept_attestation(
    attest_id: str, body: ReviewBody, db: AsyncSession = Depends(get_db)
):
    r = await db.execute(
        select(SupplierAttestation).where(SupplierAttestation.id == uuid.UUID(attest_id))
    )
