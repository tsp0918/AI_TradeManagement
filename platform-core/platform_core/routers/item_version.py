"""品目バージョン管理 + 仕様変更コンプライアンス影響検知 API ルーター。
...
def _assess_diff(prev: ItemVersion | None, curr: ItemVersion) -> list[dict]:
    ...
    # US コンテンツ率変更（De Minimis 閾値 10%/25% を跨ぐかチェック）
...
class WebhookPayload(BaseModel):
    ...
    item_id: str
    source_system: str  # "webhook_plm" | "webhook_sds" | "webhook_erp"
    source_ref: str | None = None
    change_category: str | None = None
    change_reason: str | None = None
    version_label: str | None = None
...
async def _create_version_and_events(
    ...
    supplier_ids_val = (
        data.supplier_ids if isinstance(data.supplier_ids, list) else None
    )
...
async def list_items(
    ...
    """plat_item に登録されている品目一覧と現行バージョンを返す。"""
    stmt = select(Item)
    if q:
...
async def get_item_detail(item_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    """品目詳細 + 全バージョン履歴 + コンプライアンスイベント。"""
...
async def create_version(
    ...
    """品目の新バージョンを登録し、コンプライアンス影響を自動アセスメントする。"""
    item = await db.get(Item, item_id)
    if not item:
        raise HTTPException(404, "品目が見つかりません")
...
async def receive_webhook(
    ...
    """外部システム（PLM・SDS DB・ERP）からの仕様変更通知を受信する。

    item_id は plat_item の UUID。外部システムは事前に品目を登録しておく必要がある。
    """
    try:
...
async def set_in_review(event_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    e = await db.get(ComplianceChangeEvent, event_id)
    if not e:
        raise HTTPException(404, "イベントが見つかりません")
    e.status = "in_review"
